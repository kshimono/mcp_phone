...
graph TB
    ...
    StartTimer --> ScheduleReminder[5分後にリマインダー<br/>タスクをスケジュール]
    ScheduleReminder --> WaitPayment{依頼者の<br/>アクション}

    WaitPayment -->|5分経過<br/>未決済| SendPaymentReminder[決済リマインダーSMS送信<br/>残り時間: 5分]
    SendPaymentReminder --> UpdateReminderStatus[ステータス更新<br/>payment_reminder_sent]
    UpdateReminderStatus --> WaitPayment
...
stateDiagram-v2
    ...
    payment_pending --> payment_reminder_sent: 5分経過(リマインダー送信)
    payment_reminder_sent --> paid: 決済完了
    payment_pending --> paid: 決済完了
    payment_pending --> cancelled_timeout: 10分タイムアウト
    payment_reminder_sent --> cancelled_timeout: 10分タイムアウト
